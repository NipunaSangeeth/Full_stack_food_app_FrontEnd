...
//import { imageURL } from "../utils/styles";
  ...
  return (
    <div className="bg-lightOverlay hover:drop-shadow-lg backdrop-blur-md rounded-xl flex items-center justify-between relative px-4 py-2 w-full md:w-340 md:min-w-350 gap-3">
      ...
      <div className="relative pt-12">
        <p className="text-xl text-headingColour font-semibold">
          {data.product_name}
        </p>
        ...
        <motion.div
          {...buttonClick}
          onClick={sendToCart}
          className="w-8 rounded-full bg-red-500 flex items-center justify-center absolute -top-4 right-2 cursor-pointer"
        >
          <IoBasket className="text-xl text-primary" />
        </motion.div>
      </div>
    </div>
  );
