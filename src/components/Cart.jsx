...
import { setCartOff } from "../context/action/displayCartAction";
import {
  BiChevronsRight,
  FcClearFilters,
  HiCurrencyRupee,
...
import { setCartItems } from "../context/action/cartAction";
  ...
  return (
    ...
    >
      <div className="w-full flex items-center justify-between py-4 pb-12 px-6">
        ...
        <p className="text-2xl text-headingColour font-semibold">Your Cart</p>
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6 gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            <div className="flex flex-col w-full items-start justify-start gap h-[65%] overflow-y-scroll scrollbar-none px-4 ">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
            ...
            <div className="bg-zinc-800 rounded-t-[40px] w-full h-[10%] flex flex-col items-center justify-center px-4 py-48 gap-3 ">
              <div className="w-full flex items-center justify-evenly py-10">
                <p className="text-3xl text-zinc-500 font-semibold -mt-14 ">
                  Total
                </p>
                <p className="text-3xl text-orange-500 font-semibold flex items-center justify-center -mt-14">
              ...
              <motion.button
                {...buttonClick} // cheackOut Button part
                className="bg-orange-400 w-[70%] px-4 py-3 text-xl text-headingColour font-semibold hover:bg-orange-500 drop-shadow-md rounded-2xl -mt-10"
